...
const bluetoothEventEmitter = new NativeEventEmitter(BluetoothMeshModule);
...
class BluetoothService {
  ...
  private setupEventListeners(): void {
    ...
    this.eventSubscriptions.push(
      bluetoothEventEmitter.addListener('ConnectionStateChanged', (connection) => {
        ...
        if (device) {
          ...
          this.onConnectionStateChanged?.(connection.deviceId, connection.status);
        }
      })
    );
    
    this.eventSubscriptions.push(
      bluetoothEventEmitter.addListener('MessageReceived', async (messageData) => {
        try {
          const device = this.discoveredDevices.get(messageData.deviceId);
          if (device && device.publicKey) {
            const decryptedData = await cryptoService.decrypt(messageData.data, device.publicKey);
  ...
  async sendMessage(deviceId: string, message: Message): Promise<void> {
    try {
      ...
      if (device && device.publicKey) {
          const encryptedData = await cryptoService.encrypt(JSON.stringify(message), device.publicKey);
