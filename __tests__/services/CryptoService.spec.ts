...
describe('CryptoService', () => {
  ...
  it('should establish a session key', async () => {
    ...
    expect(CryptoModule.generateSharedSecret).toHaveBeenCalledWith(remotePublicKey);
  });
  ...
  it('should encrypt and decrypt a message', async () => {
    ...
    await cryptoService.establishSessionKey(remotePublicKey);
    const encryptedMessage = await cryptoService.encrypt(message, remotePublicKey);
    const decryptedMessage = await cryptoService.decrypt(encryptedMessage, remotePublicKey);
